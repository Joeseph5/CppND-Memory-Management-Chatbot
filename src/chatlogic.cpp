...
ChatLogic::~ChatLogic()
{
    ...
    // delete all nodes // unique_ptr离开作用域默认调用delete
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            edge->SetChildNode((*childNode).get()); // childNode类型为智能指针的迭代器
                            edge->SetParentNode((*parentNode).get());
                            // _edges.push_back(edge);
                            // _edges.push_back(std::move(edge)); // move(edge)会转移edge资源，导致无法AddEdgeToChildNode等操作
    ...
    // pass the `ChatBot` instance into the root node by move senmatincs
